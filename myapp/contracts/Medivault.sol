...
contract Medivault {
    ...
    constructor() public {
        superAdmins[msg.sender] = SuperAdmin({
            name: "Super Admin"
        })
    }
    ...
    function addSuperAdmin(address pubkey, string calldata name) public {
        require(checkSuperAdmin(msg.sender) == true, "You are not a super admin");
        require(checkSuperAdmin(pubkey) == false, "Super admin already exists");
        superAdmins[pubkey] = SuperAdmin({
            name: name
        });
    }
    ...
    function addAdmin(
        ...
        require(checkSuperAdmin(msg.sender) == true, "You are not a allowed to add an admin");
    ...
    function editAdmin(
        ...
        require(checkSuperAdmin(msg.sender) == true, "You are not a allowed to edit an admin");
    ...
    function deleteAdmin(address pubkey) public {
        require(checkSuperAdmin(msg.sender) == true, "You are not a allowed to delete an admin");
        require(checkAdmin(pubkey) == true, "Admin does not exist");
        delete admins[pubkey];
    }

    // function to fetch a single admin
    function getAdmin(address pubkey) public view returns (Admin) {
        require(checkSuperAdmin(msg.sender) == true, "You are not a allowed to fetch an admin");
        require(checkAdmin(pubkey) == true, "Admin does not exist");
        return admins[pubkey];
    }

    // function to fetch all admins 
    function getAdmins() public view returns (Admin[]) {
        require(checkSuperAdmin(msg.sender) == true, "You are not a allowed to perform this opertation");
        return admins;
    }
...
// -------------------------------------------------------------------------------------------
    ...
    function editDoctor(
        ...
        require(msg.sender != admins[pubkey].institution_name, "You do not have the permission to edit this doctor");
        require(bytes(name).length > 0, "Name cannot be empty");
        require(bytes(specialization).length > 0, "Specialization cannot be empty");
        require(bytes(contact).length > 0, "Contact cannot be empty");
        doctors[pubkey] = Doctor({
    ...
    function deleteDoctor(address pubkey) public {
        require(checkAdmin(msg.sender) == true, "You are not an admin");
        require(checkDoctor(pubkey) == true, "Doctor does not exist");
        require(msg.sender != admins[pubkey].institution_name, "You do not have the permission to delete this doctor");
        delete doctors[pubkey];
    }

    // function to fetch a single doctor under the admin
    function getDoctor(address pubkey) public view returns (Doctor) {
        require(checkAdmin(msg.sender) == true, "You are not an admin");
        require(checkDoctor(pubkey) == true, "Doctor does not exist");
        require(msg.sender == doctors[pubkey].institution_name, "You do not have the permission to fetch this doctor");
        return doctors[pubkey];
    }

    // function to fetch a all doctors under the admin's instiution name 
    function getDoctors() public view returns (Doctor[]) {
        require(checkAdmin(msg.sender) == true, "You are not an admin");
        Doctor[] _doctors = [];
        for (address pubkey in doctors) {
            if (admins[pubkey].institution_name == msg.sender) {
...
// -------------------------------------------------------------------------------------------
    ...
    function getPatient(address pubkey) public view returns (Patient) {
        require(checkDoctor(msg.sender) == true, "Doctor does not exist");
    ...
    function checkReadAccess(bytes32 record_hash) public view returns (bool) {
        ...
        if(records[record_hash].read_allowed_doctors.length > 0) {
            for (address pubkey in records[record_hash].read_allowed_doctors) {
    ...
    function checkWriteAccess(bytes32 record_hash) public view returns (bool) {
        ...
        if(records[record_hash].write_allowed_doctors.length > 0) {
            for (address pubkey in records[record_hash].write_allowed_doctors) {
                if (pubkey == msg.sender) {
                    return true;
                }
            }
    ...
    function getRecord(bytes32 record_hash) public view returns (Record) {
        require(checkDoctor(msg.sender) == true, "Doctor does not exist");
        require(checkRecord(record_hash) == true, "Record does not exist");
        require(checkReadAccess(record_hash) == true, "You do not have access to read this record");
    ...
    function editRecord(
        ...
        require(checkDoctor(msg.sender) == true, "Doctor does not exist");
        require(checkRecord(record_hash) == true, "Record does not exist");
        require(checkWriteAccess(record_hash) == true, "You do not have access to write this record");
...
// -------------------------------------------------------------------------------------------
    ...
    function fetchRecords() public view returns (Record[]) {
        require(checkPatient(msg.sender) == true, "Patient does not exist");
        Record[] _records = [];
        for (bytes32 record_hash in records) {
            if (records[record_hash].patient == msg.sender) {
                _records.push(records[record_hash]);
            }
        }
        return _records;
    }
    ...
    function fetchRecord(bytes32 record_hash) public view returns (Record) {
        require(checkPatient(msg.sender) == true, "Patient does not exist");
        require(checkRecord(record_hash) == true, "Record does not exist");
        require(records[record_hash].patient != msg.sender, "You do not have access to this record");
        return records[record_hash];
    }
    ...
    function fetchNomineeRecords(address pubkey) public view returns (Record[]) {
        require(checkPatient(msg.sender) == true, "Patient does not exist");
        require(checkPatient(pubkey) == true, "Nominee does not exist");
        Record[] _records = [];
        for (bytes32 record_hash in records) {
            if (records[record_hash].patient == pubkey) {
                _records.push(records[record_hash]);
            }  
        }
        return _records;
